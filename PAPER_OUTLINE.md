...
2. Crossover scale κ_c extracted purely from data (no exponent fitting)
3. Detection generalizes across system sizes (scale-invariant)
4. Gradient/temporal features outperform traditional α,β estimation
...
- Normalize raw scores to D_ML ∈ [0, 1]:
  - D_ML = 0 for pure KPZ (κ=0)
  - D_ML → 1 for asymptotic MBE behavior (large κ)
...
|-----------|-------|-------|
| Crossover scale κ_c | 0.759 | ± 0.048 |
| Sharpness exponent γ | 1.51 | ± 0.16 |
| Fit quality R² | 0.964 | — |

**Raw scores:**
- κ=0.0 (pure KPZ): score = +0.105
- κ=0.76 (crossover): score ≈ +0.03
- κ=10.0 (MBE-like): score = -0.018

**Key findings:**
1. **Monotonic behavior**: D_ML increases smoothly from 0 to 1 as κ increases
2. **Quantitative crossover scale**: κ_c = 0.76 ± 0.05 identifies where KPZ and MBE physics contribute equally
3. **Well-described by saturation curve**: R² = 0.96 indicates the distance metric has a predictable functional form
4. **Data-driven, not theory-imposed**: The crossover scale and sharpness emerge from the ML geometry, not from fitting exponents

**Physical interpretation:**
The crossover scale κ_c can be understood theoretically. In the hybrid equation:
∂h/∂t = ν∇²h + (λ/2)(∇h)² - κ∇⁴h + η

The KPZ nonlinearity dominates at scales ℓ < ℓ_× where ℓ_× ~ (κ/λ)^(1/2). For κ_c ≈ 0.76 and λ=1, this gives ℓ_× ≈ 0.87, suggesting the ML metric captures the competition between second-order (KPZ) and fourth-order (MBE) dynamics at the lattice scale.

**The Novel Claim:**

> **Universality class proximity can be quantified directly from finite-size simulation data without fitting scaling exponents.**

Traditional RG analysis requires:
1. Large system sizes for clean power laws
2. Careful fitting of α, β with systematic errors
3. Comparison to theoretical predictions

The ML distance metric provides:
- A continuous, data-driven proximity measure
- Works at finite L (tested at L=128)
- Captures crossover behavior smoothly
- Gives quantitative crossover parameters (κ_c, γ)

**Figures:**
- `universality_distance_main.png`: Main result figure showing D_ML(κ) with fit
- `universality_distance.png`: Two-panel figure with raw scores and normalized distance
